...
const unsigned long BLINK_INTERVAL = 3000;      // Clignement toutes les 1.6s
...
void setup() {
  ...
  Serial.println("Systeme pret ! Mode Mini-Canvas actif (Serial + Clignement).");
}
...
void checkSerial() {
  if (Serial.available() > 0) {
    ...
    if (commaIndex > 0) {
      int inX = input.substring(0, commaIndex).toInt();
      int inY = input.substring(commaIndex + 1).toInt();

      int dx = inX - CENTER_X;
      int dy = inY - CENTER_Y;
      float distance = sqrt(dx * dx + dy * dy);
      int maxDist = 120 - PUPIL_RADIUS;

      if (distance > maxDist) {
        targetX = CENTER_X + (dx / distance) * maxDist;
        targetY = CENTER_Y + (dy / distance) * maxDist;
      } else {
        targetX = inX;
        targetY = inY;
      }
...
void loop() {
  ...
  // 1. DECLENCHEMENT DU CLIGNEMENT (Indépendant)
  ...
  // 4. MOTEUR DE RENDU HYBRIDE (Delta Redraw + Mini-Canvas)
